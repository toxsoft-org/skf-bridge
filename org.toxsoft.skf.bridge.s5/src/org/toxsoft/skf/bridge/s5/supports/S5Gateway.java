...
class S5Gateway
  ...
  S5Gateway( S5BackendGatewaySingleton aOwner, ISkGatewayConfiguration aConfiguration ) {
    ...
    // owner.objectsBackend().addObjectsInterceptor( new S5GatewayObjectsInterceptor( this ), 0 );
    owner.currdataBackend().addCurrDataInterceptor( this, 0 );
    owner.histdataBackend().addHistDataInterceptor( this, 0 );
    // owner.lobsBackend().addLobsInterceptor( new S5GatewayLobsInterceptor( this ), 0 );
  ...
  public void afterCloseSession( Skid aSessionID ) {
    ...
    // От локального сервера отключился один клиентов. Требуется перенастроить порт приема текущих данных от сервера,
    // так как совокупный список получаемых данных может быть изменен (нет больше необходимости в некоторых данных)
  ...
  private void tryOpenRemote() {
    ...
    try {
      ...
      localToRemoteCurrdataPort =
          new S5GatewayCurrDataPort( "localToRemote", localRtDataService, remoteRtDataService, logger ); //$NON-NLS-1$
      remoteToLocalCurrdataPort =
          new S5GatewayCurrDataPort( "remoteToLocal", remoteRtDataService, localRtDataService, logger ); //$NON-NLS-1$
  ...
  public static ISkConnection openRemoteConnection( ISkConnection aConnection, ISkBackendProvider aProvider,
    ...
    // Настройки имеющие значение для подключения к удаленным серверам. Игнорируются для локальных соединений
  ...
  private void synchronizeCmdExecutors( IGwidList aCmdGwids ) {
    ...
    try {
      ...
      // Время выполнения запроса
      Long traceTime = Long.valueOf( System.currentTimeMillis() - traceStartTime );
      // Ошибка регистрации исполнителей команд на удаленном сервере. Соединение будет закрыто
  ...
  private void createRtdataChannels( IGwidList aDataGwids ) {
    ...
    if( aDataGwids.size() > 0 ) {
      ...
      writeHistData.clear();
      // Создание новых каналов хранимых данных. localToRemoteCurrdataPort возвращает только уникальные, без дублей
  ...
  private static void configureCurrdataPortForImport( IS5BackendCurrDataSingleton aCurrdataBackend,
    ...
    // Фильтрация данных значения которых не формируются другими (не gateway) писателями
