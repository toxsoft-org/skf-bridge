...
public class RefbookGenerator {
  ...
  static IDtoAttrInfo ATRINF_SI_MASKS_DRV_OBJ_ID = DtoAttrInfo.create2( RBATRID_SI_MASKS_DRV_OBJ_ID, DDEF_STRING, //
      TSID_NAME, "AI объект производного", TSID_DESCRIPTION, "Объект AI производного данного", //
      TSID_DEFAULT_VALUE, AvUtils.AV_STR_EMPTY //
  );
  ...
  static IDtoAttrInfo ATRINF_SI_MASKS_DRV_FLAG_DATA_ID =
      DtoAttrInfo.create2( RBATRID_SI_MASKS_DRV_FLAG_DATA_ID, DDEF_STRING, //
          TSID_NAME, "флаг производного данного AI", TSID_DESCRIPTION, "Флаг производного данного AI", //
          TSID_DEFAULT_VALUE, AvUtils.AV_STR_EMPTY //
      );
  ...
  public void createPoligonBitMaskRefbook() {
    ...
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___CTRLSYSTEM_ENGINEBLOCK, "Есть блокировка", "Есть блокировка
    // привода",
    ...
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___CTRLSYSTEM_WATERINGED, "Вода в ГЭД", "Вода в ГЭД", "rtdWB_AUTO",
    // 13,
    // TsLibUtils.EMPTY_STRING, TsLibUtils.EMPTY_STRING );
    ...
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___CTRLSYSTEM_ARMCONNECT, "Связь с АРМ", "Связь с АРМ", "rtdNetWord",
    // 7,
    // TsLibUtils.EMPTY_STRING, TsLibUtils.EMPTY_STRING );
    ...
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___CTRLSYSTEM_ENABLEARMCONNECTALARM, "Разрешение контроля связи с
    // АРМ",
    ...
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___IRREVERSIBLEENGINE_IMITATION, "Имитация", "Включен режим
    // имитации",
    ...
    // "rtdStateWord", 4, TsLibUtils.EMPTY_STRING, TsLibUtils.EMPTY_STRING );
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___IRREVERSIBLEENGINE_ON, "Включен", "Электропривод включен
    // логически",
    // "rtdStateWord", 5, TsLibUtils.EMPTY_STRING, TsLibUtils.EMPTY_STRING );
    ...
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___VALVE_PWR, "Питание", "Питание в норме/отсутствует",
    // "rtdStateWord",
    ...
    // "rtdStateWord", 3, "Отключена", "Включена" );
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___VALVE_READY, "Готовность", "Готовность привода", "rtdStateWord",
    // 4,
    // TsLibUtils.EMPTY_STRING, TsLibUtils.EMPTY_STRING );
    ...
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___MAINSWITCH_CTRLFAILURE, "Контроля нет", "Нет напряжения
    // управления",
    ...
    // TsLibUtils.EMPTY_STRING );
    // addBitMaskRbItem( rbBitMasks, ITEMID_BITMASK___MAINSWITCH_ONFAILURE, "Не включился", "Не включился",
    // "rtdErrorWord",
  ...
  private void addBitMaskRbItem( ISkRefbook aRefbook, String aItemId, String aName, String aDescr, String aWordId,
      int aBitNumber, String aFallStr, String aFrontStr, boolean isRriParam, String aiDataId, String aiStrid ) {
    RefookItemBuider b = new RefookItemBuider( aItemId, aName, aDescr, aRefbook );
    ...
    b.setValue( ATRINF_SI_MASKS_DRV_FLAG_DATA_ID.id(), AvUtils.avStr( aiDataId ) );
    b.setValue( ATRINF_SI_MASKS_DRV_OBJ_ID.id(), AvUtils.avStr( aiStrid ) );
    IDtoFullObject rbItem = b.buildItem();
    aRefbook.defineItem( rbItem );
  }

  /**
   * Импорт справочника масок для проекта Самсунг из файла "Проект SI, импорт масок AI.ods"<br>
  ...
  private static int COLUMN_SI_BITMASK_EVENT_UP     = COLUMN_SI_BITMASK_DRV_CLASS_ID + 5;
  private static int COLUMN_SI_BITMASK_EVENT_DN     = COLUMN_SI_BITMASK_DRV_CLASS_ID + 6;
  ...
  private void fillSiMasksRbItems( ISkRefbook aRbSiBitMasks, Sheet aMaskSheet ) {
    ...
    for( int rowNum = 2; rowNum < aMaskSheet.getRowCount(); rowNum++ ) {
      ...
      String eventUp = getStringValue( aMaskSheet, COLUMN_SI_BITMASK_EVENT_UP, rowNum );
      String eventDn = getStringValue( aMaskSheet, COLUMN_SI_BITMASK_EVENT_DN, rowNum );
      // create itemId
      // exclude word id
      String itemId = drvClassId + "." + drvObjId + "." + drvDataId;
      // fill refbook
      addSiBitMaskRbItem( aRbSiBitMasks, itemId, drvClassId, drvObjId, drvDataId, srcDataId, Integer.parseInt( bitNum ),
          eventUp, eventDn );
    }
  }

  private void addSiBitMaskRbItem( ISkRefbook aRefbook, String aItemId, String aDrvClassId, String aDrvObjId,
      String aDrvFlagId, String aSrcDataId, int aBitNumber, String aEventUp, String aEventDn ) {
    ...
    b.setValue( ATRINF_BITMASK_ON.id(), AvUtils.avStr( aEventUp ) );
    b.setValue( ATRINF_BITMASK_OFF.id(), AvUtils.avStr( aEventDn ) );
    IDtoFullObject rbItem = b.buildItem();
    aRefbook.defineItem( rbItem );

  }

  public void importCommonBitMaskRefbook( File aRefbookFile ) {
    ISkRefbookService rbServ = conn.coreApi().getService( ISkRefbookService.SERVICE_ID );
    // ask user to rewrite existing refbook
  ...
  private void fillClassRbItems( ISkRefbook rbBitMasks, Sheet classSheet, String className ) {
    ...
    for( int rowNum = 2; rowNum < classSheet.getRowCount(); rowNum++ ) {
      ...
      String aiDataId = getStringValue( classSheet, COLUMN_AI_DATA_ID, rowNum );
      String aiStrid = getStringValue( classSheet, COLUMN_AI_STRID, rowNum );
      // create itemId
      // exclude word id
      ...
      addBitMaskRbItem( rbBitMasks, itemId, name, descr, idw, Integer.parseInt( bitNum ), off, on,
          identificator.startsWith( "rri" ), aiDataId, aiStrid );
    }
  }
  ...
  private static int COLUMN_AI_DATA_ID            = 7;
  private static int COLUMN_AI_STRID              = 8;
